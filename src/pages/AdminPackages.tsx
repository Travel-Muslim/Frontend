...
export default function AdminPackages() {
  ...
  const filtered = useMemo(() => {
    ...
    return packages.filter((p) => {
      const haystack = [p.title, p.location, p.departure, p.airline, p.airport].filter(Boolean).join(" ").toLowerCase();
          ...
          <div className="ap-card-head">
            ...
            <div className="ap-actions">
              <div className="ap-search">
                ...
                <input type="text" placeholder="Cari Paket" value={query} onChange={(e) => setQuery(e.target.value)} />
              </div>
              <button type="button" className="ap-add-btn" onClick={() => navigate("/admin/packages/new")}>
                <IconPlus />
                Tambahkan Paket
              </button>
            </div>
              ...
              {loading ? (
                ...
                filtered.map((p) => (
                  <div className="ap-row" key={p.id}>
                    ...
                    <span data-label="Keberangkatan">{p.departure || p.period?.join(", ")}</span>
                    <span data-label="Maskapai">{p.airline}</span>
                    <span data-label="Harga">{p.price ? `Rp${p.price.toLocaleString("id-ID")}` : "-"}</span>
                    <span className="ap-actions-row" data-label="Aksi">
                      ...
                      <button type="button" className="ap-btn ap-btn-delete" aria-label="Hapus paket" onClick={() => handleDelete(Number(p.id))}>
                        <IconTrash />
                      </button>
                    </span>
                  </div>
