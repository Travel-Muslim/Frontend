...
const ARTICLE_TEMPLATE: Required<Pick<Article, "title" | "date" | "displayDate" | "content" | "image" | "gallery" | "blocks">> = {
  title: "",
  date: "",
  displayDate: "",
  content: "",
...
function IconCalendar() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
      <rect x="4" y="6" width="16" height="14" rx="3" fill="none" stroke="#f28b95" strokeWidth="2" />
      <path d="M8 4v4M16 4v4M4 10h16" fill="none" stroke="#f28b95" strokeWidth="2" strokeLinecap="round" />
    </svg>
  );
}
...
function IconBold() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
      <path d="M7 5h6a3 3 0 0 1 0 6H7zm0 6h7a3 3 0 0 1 0 6H7z" fill="none" stroke="#b08cf2" strokeWidth="2" />
    </svg>
  );
}
...
function IconItalic() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
      <path d="M10 5h6M8 19h6m1-14-5 14" fill="none" stroke="#b08cf2" strokeWidth="2" strokeLinecap="round" />
    </svg>
  );
}
...
function IconAlignLeft() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
      <path d="M5 7.5h14" stroke="#b08cf2" strokeWidth="2" strokeLinecap="round" />
      <path d="M5 12h10" stroke="#b08cf2" strokeWidth="2" strokeLinecap="round" />
      <path d="M5 16.5h14" stroke="#b08cf2" strokeWidth="2" strokeLinecap="round" />
    </svg>
  );
}
...
function IconAlignCenter() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
      <path d="M6 7.5h12" stroke="#b08cf2" strokeWidth="2" strokeLinecap="round" />
      <path d="M8 12h8" stroke="#b08cf2" strokeWidth="2" strokeLinecap="round" />
      <path d="M6 16.5h12" stroke="#b08cf2" strokeWidth="2" strokeLinecap="round" />
    </svg>
  );
}
...
function IconAlignJustify() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
      <path d="M5 7.5h14" stroke="#b08cf2" strokeWidth="2" strokeLinecap="round" />
      <path d="M5 12h14" stroke="#b08cf2" strokeWidth="2" strokeLinecap="round" />
      <path d="M5 16.5h14" stroke="#b08cf2" strokeWidth="2" strokeLinecap="round" />
    </svg>
  );
}
...
function IconList() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
      ...
      <path d="M10 8h8M10 12h8M10 16h8" stroke="#b08cf2" strokeWidth="2" strokeLinecap="round" />
    </svg>
  );
}
...
function IconImage() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
      <rect x="4" y="6" width="16" height="12" rx="2.2" stroke="#b08cf2" strokeWidth="2" fill="none" />
      <path d="M8 13.5 11 10l3 4 2-1.8L18 14" fill="none" stroke="#b08cf2" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
  ...
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
      <path
        ...
        strokeLinecap="round"
      />
      <path d="M10 14 14 10" stroke="#b08cf2" strokeWidth="2" strokeLinecap="round" />
    </svg>
  );
}
...
export default function AdminArticleForm() {
  ...
  const toHtml = (val: string) => (val && val.includes("<") ? val : val ? `<p>${val}</p>` : "");
  const [contentHtml, setContentHtml] = useState<string>(toHtml(ARTICLE_TEMPLATE.content));
  const [imageSrc, setImageSrc] = useState(ARTICLE_TEMPLATE.image);
  const [gallery, setGallery] = useState<string[]>(ARTICLE_TEMPLATE.gallery || []);
  ...
  const article = useMemo(
    () => (isEditMode ? articles.find((a) => String(a.id) === String(articleKey)) : undefined),
  ...
  const previewContent = useMemo(() => contentHtml || article?.content || ARTICLE_TEMPLATE.content || "Isi artikel akan tampil di sini.", [article?.content, contentHtml]);
  const previewLink = "";
  const previewGallery = (gallery && gallery.length > 0 ? gallery : article?.gallery) || ARTICLE_TEMPLATE.gallery || [];
  const [bold, setBold] = useState(false);
  const [italic, setItalic] = useState(false);
  const [underline, setUnderline] = useState(false);

  const textareaStyle: React.CSSProperties = {
    fontWeight: bold ? 800 : fontWeight === "Bold" ? 800 : fontWeight === "Semi-Bold" ? 700 : 400,
    fontSize,
  ...
  const handleSave = async () => {
    const payload: Article = {
      ...
      displayDate: formatDisplayDate(String(date || article?.displayDate || ARTICLE_TEMPLATE.displayDate || ARTICLE_TEMPLATE.date)),
      ...
      gallery: gallery.length ? gallery : article?.gallery || ARTICLE_TEMPLATE.gallery || [],
      ...
      <main className="ad-main">
        <header className="ad-topbar">
          <div className="ad-topbar-left">
            ...
            <button className="ap-back-btn" type="button" onClick={() => navigate("/admin/articles")} aria-label="Kembali">
              <IconArrowLeft />
              <span>Kembali</span>
            </button>
            <h1>{isEditMode && hasArticle ? "Edit Artikel" : "Tambahkan Artikel"}</h1>
          </div>
        ...
        <section className="aaf-card aaf-meta-card">
          <div className="aaf-card-head">
            ...
            <p className="apd-sub">Isi kolom di bawah ini secara lengkap untuk {(isEditMode && hasArticle) ? "memperbarui" : "membuat"} draft artikel.</p>
          </div>
        ...
        <section className="aaf-card aaf-content-card">
          <div className="aaf-card-head">
            <h2>Isi Artikel</h2>
            <p className="apd-sub">
              Tulis konten artikel Anda di sini. Pastikan narasi mengalir dengan baik, informatif, dan relevan dengan kategori yang dipilih.
            </p>
          </div>
          ...
          <div className="aaf-toolbar">
            <div className="aaf-select">
              ...
              <select value={fontWeight} onChange={(e) => setFontWeight(e.target.value as TextWeight)}>
            ...
            <button type="button" aria-label="Tambahkan gambar" onClick={() => contentImageInputRef.current?.click()}>
              <IconImage />
            </button>
          </div>
          <input
          ...
          style={{ ...textareaStyle, direction: "ltr", unicodeBidi: "normal" as React.CSSProperties["unicodeBidi"] }}
        ...
        <div className="aaf-preview-actions">
          <button type="button" className="apd-btn-secondary aaf-preview-btn" onClick={() => setPreviewOpen(true)}>
            Tampilkan Detail Preview
          </button>
          <div className="aaf-footer-row">
            <button type="button" className="apd-btn apd-btn-cancel" onClick={() => navigate("/admin/articles")}>
              Batalkan Artikel
            </button>
            <button type="button" className="apd-btn apd-btn-save" onClick={handleSave}>
              {isEditMode && hasArticle ? "Simpan Artikel" : "Tambahkan Artikel"}
            </button>
          </div>
        </div>
        ...
        {successModal && (
          <div className="ap-modal">
            <div className="ap-modal-card">
              ...
              <h3>Artikel {(isEditMode && hasArticle) ? "Diperbarui" : "Tersimpan"}</h3>
              <p>Artikel berhasil {(isEditMode && hasArticle) ? "diperbarui" : "disimpan"}.</p>
            ...
            <div className="ap-preview-card">
              ...
              <div className="aaf-preview">
                <div className="aaf-preview-meta">
                  ...
                  {previewLink && (
                    <a className="aaf-preview-link" href={previewLink} target="_blank" rel="noreferrer">
                      {previewLink}
                    </a>
                  )}
                </div>
                ...
                <div className="aaf-preview-body" dangerouslySetInnerHTML={{ __html: previewContent }} />
                {previewGallery.length > 0 && (
                  <div className="aaf-preview-gallery">
                    {previewGallery.map((src, idx) => (
                      <img key={`${src}-${idx}`} src={src} alt={`${previewTitle} ${idx + 1}`} />
                    ))}
                  </div>
                )}
              </div>
