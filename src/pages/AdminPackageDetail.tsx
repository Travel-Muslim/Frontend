...
function IconArrowLeft() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
      <path d="M14.5 6.5 8 12l6.5 5.5" fill="none" stroke="#7b5ad3" strokeWidth="2.2" strokeLinecap="round" />
      <path d="M8.5 12H19" fill="none" stroke="#7b5ad3" strokeWidth="2.2" strokeLinecap="round" />
    </svg>
  );
}
...
function IconUpload() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
      <path d="M12 16V5.5" fill="none" stroke="#f28b95" strokeWidth="2.2" strokeLinecap="round" />
      <path d="m8.5 9 3.5-3.5L15.5 9" fill="none" stroke="#f28b95" strokeWidth="2.2" strokeLinecap="round" />
      <path d="M6 16.5v2.5h12v-2.5" fill="none" stroke="#f28b95" strokeWidth="2.2" strokeLinecap="round" />
    </svg>
  );
}
...
function IconCheck() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
      <path d="M5 12.5 10 17.5 19 7" fill="none" stroke="#ffffff" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
...
function IconClose() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
      <path d="m7 7 10 10M17 7 7 17" fill="none" stroke="#4a4a4a" strokeWidth="2.2" strokeLinecap="round" />
    </svg>
  );
}
...
export default function AdminPackageDetail() {
  ...
  useEffect(() => {
    ...
    fetchPackage(pkgId).then((data) => {
      ...
      setForm({
        ...
        itinerary: {
          destination: data.itinerary?.map((d) => d.destinasi?.join(", ") || "").filter(Boolean) ?? [""],
          food: data.itinerary?.map((d) => d.makan?.join(", ") || "").filter(Boolean) ?? [""],
          mosque: data.itinerary?.map((d) => d.masjid?.join(", ") || "").filter(Boolean) ?? [""],
          transport: data.itinerary?.map((d) => d.transportasi?.join(", ") || "").filter(Boolean) ?? [""],
        },
      });
  ...
  const updateItinerary = (key: keyof typeof form.itinerary, idx: number, value: string) => {
    setForm((prev) => {
  ...
  const cancelCta = isCreateMode ? "Batalkan Paket Tour" : "Batalkan Edit Paket";
  const successTitle = isCreateMode ? "Paket Berhasil Ditambahkan" : "Paket Berhasil Diperbarui";
  const successDesc = isCreateMode ? "Data paket baru sudah tersimpan." : "Data paket berhasil diperbarui.";

  const handleSave = async () => {
    const priceNumber = Number(String(form.price).replace(/[^0-9]/g, "")) || undefined;
    const itinerary = Array.from({ length: maxDays }).map((_, idx) => ({
      day: `Hari ${idx + 1}`,
      destinasi: form.itinerary.destination[idx] ? [form.itinerary.destination[idx]] : [],
      makan: form.itinerary.food[idx] ? [form.itinerary.food[idx]] : [],
      masjid: form.itinerary.mosque[idx] ? [form.itinerary.mosque[idx]] : [],
      transportasi: form.itinerary.transport[idx] ? [form.itinerary.transport[idx]] : [],
    }));
    await savePackage(
      {
        title: form.name,
      ...
      <main className="ad-main">
        <header className="ad-topbar">
          <div className="ad-topbar-left">
            ...
            <button className="ap-back-btn" type="button" onClick={() => navigate("/admin/packages")} aria-label="Kembali">
              <IconArrowLeft />
              <span>Kembali</span>
            </button>
            <h1>{pageTitle}</h1>
        ...
        <section className="apd-card">
          ...
          <p className="apd-sub">Isi kolom di bawah ini secara lengkap untuk membuat paket destinasi baru.</p>
          <form
            className="apd-form"
            onSubmit={(e) => {
              e.preventDefault();
          ...
          >
            <div className="apd-grid">
              ...
              <label className="apd-field">
                <span>Lokasi*</span>
                <input type="text" value={form.location} onChange={(e) => setForm((prev) => ({ ...prev, location: e.target.value }))} required />
              </label>
              <label className="apd-field">
                <span>Benua*</span>
                <input type="text" value={form.continent} onChange={(e) => setForm((prev) => ({ ...prev, continent: e.target.value }))} required />
              </label>
              <label className="apd-field">
                <span>Maskapai*</span>
                <input type="text" value={form.airline} onChange={(e) => setForm((prev) => ({ ...prev, airline: e.target.value }))} required />
              </label>
              <label className="apd-field">
                <span>Bandara*</span>
                <input type="text" value={form.airport} onChange={(e) => setForm((prev) => ({ ...prev, airport: e.target.value }))} required />
              </label>
              <label className="apd-field">
                <span>Periode Keberangkatan*</span>
                <input type="text" value={form.departure} onChange={(e) => setForm((prev) => ({ ...prev, departure: e.target.value }))} required />
              </label>
              <label className="apd-field">
                <span>Harga*</span>
                <input type="text" value={form.price} onChange={(e) => setForm((prev) => ({ ...prev, price: e.target.value }))} required />
              </label>
            </div>

            <div className="apd-image-block">
              <h3>Gambar Paket</h3>
              <div className={`apd-image-placeholder ${imageSrc ? "has-image" : ""}`} onClick={() => fileInputRef.current?.click()}>
                {imageSrc ? (
                ...
                />
                <button type="button" className="apd-upload-btn" onClick={() => fileInputRef.current?.click()}>
                  Pilih Gambar
                </button>
              </div>
            </div>
            ...
            <div className="apd-section">
              ...
              <p className="apd-sub">Isi kolom di bawah ini secara lengkap untuk menambahkan destinasi pada itenary paket.</p>
              {form.itinerary.destination.map((v, idx) => (
                <label className="apd-field" key={`dest-${idx}`}>
                  <span>Hari {idx + 1}*</span>
                  <input type="text" value={v} onChange={(e) => updateItinerary("destination", idx, e.target.value)} />
                </label>
              ))}
              <div className="apd-section-actions">
                <button type="button" className="apd-btn-ghost" onClick={() => removeDay("destination")}>
                  <span className="apd-btn-icon">
                    <IconMinus />
                  </span>
                  Hapus Hari
                </button>
                <button type="button" className="apd-btn-outline" onClick={() => addDay("destination")}>
                  <span className="apd-btn-icon">
                    <IconPlus />
                  </span>
                  Tambahkan Hari
              ...
              <div className="apd-actions apd-actions-inline">
                <button type="button" className="apd-btn apd-btn-save" onClick={() => setSuccessModal(true)}>
                  Simpan Perubahan
                </button>
              </div>
            </div>
            ...
            <div className="apd-section">
              ...
              <p className="apd-sub">Isi kolom di bawah ini secara lengkap untuk menambahkan tempat makan pada itenary paket.</p>
              {form.itinerary.food.map((v, idx) => (
                <label className="apd-field" key={`food-${idx}`}>
                  <span>Hari {idx + 1}*</span>
                  <input type="text" value={v} onChange={(e) => updateItinerary("food", idx, e.target.value)} />
                </label>
              ))}
              <div className="apd-section-actions">
                <button type="button" className="apd-btn-ghost" onClick={() => removeDay("food")}>
                  <span className="apd-btn-icon">
                    <IconMinus />
                  </span>
                  Hapus Hari
                </button>
                <button type="button" className="apd-btn-outline" onClick={() => addDay("food")}>
                  <span className="apd-btn-icon">
                    <IconPlus />
                  </span>
                  Tambahkan Hari
            ...
            <div className="apd-section">
              ...
              <p className="apd-sub">Isi kolom di bawah ini secara lengkap untuk menambahkan masjid pada itenary paket.</p>
              {form.itinerary.mosque.map((v, idx) => (
                <label className="apd-field" key={`mosque-${idx}`}>
                  <span>Hari {idx + 1}*</span>
                  <input type="text" value={v} onChange={(e) => updateItinerary("mosque", idx, e.target.value)} />
                </label>
              ))}
              <div className="apd-section-actions">
                <button type="button" className="apd-btn-ghost" onClick={() => removeDay("mosque")}>
                  <span className="apd-btn-icon">
                    <IconMinus />
                  </span>
                  Hapus Hari
                </button>
                <button type="button" className="apd-btn-outline" onClick={() => addDay("mosque")}>
                  <span className="apd-btn-icon">
                    <IconPlus />
                  </span>
                  Tambahkan Hari
            ...
            <div className="apd-section">
              ...
              <p className="apd-sub">Isi kolom di bawah ini secara lengkap untuk menambahkan transportasi pada itenary paket.</p>
              {form.itinerary.transport.map((v, idx) => (
                <label className="apd-field" key={`transport-${idx}`}>
                  <span>Hari {idx + 1}*</span>
                  <input type="text" value={v} onChange={(e) => updateItinerary("transport", idx, e.target.value)} />
                </label>
              ))}
              <div className="apd-section-actions">
                <button type="button" className="apd-btn-ghost" onClick={() => removeDay("transport")}>
                  <span className="apd-btn-icon">
                    <IconMinus />
                  </span>
                  Hapus Hari
                </button>
                <button type="button" className="apd-btn-outline" onClick={() => addDay("transport")}>
                  <span className="apd-btn-icon">
                    <IconPlus />
                  </span>
                  Tambahkan Hari
            ...
            <div className="apd-footer-actions">
              <button type="button" className="apd-btn apd-btn-secondary" onClick={() => setPreviewOpen(true)}>
                Tampilkan Detail Preview
              </button>
              <button type="button" className="apd-btn apd-btn-cancel" onClick={() => navigate("/admin/packages")}>
                {cancelCta}
              </button>
              <button type="button" className="apd-btn apd-btn-save" onClick={handleSave}>
                {primaryCta}
              </button>
            </div>
          </form>
        ...
        {previewOpen && (
          <div className="ap-preview-modal">
            <div className="ap-preview-card">
              <button className="ap-preview-close" type="button" onClick={() => setPreviewOpen(false)}>
                <IconClose />
              </button>
              <h3>Detail Preview</h3>
              <div className="ap-preview-hero">
                <div className="ap-preview-img">
                  {imageSrc ? <img src={imageSrc} alt={form.name || "Paket"} /> : null}
                </div>
                <div className="ap-preview-meta">
                  <h4>{form.name || pkg?.name || "Paket Baru"}</h4>
                  <div className="ap-preview-grid">
                    <div><strong>Lokasi</strong><span>{form.location}</span></div>
                    <div><strong>Durasi</strong><span>6 Hari 4 Malam</span></div>
                    <div><strong>Periode</strong><span>{form.departure}</span></div>
                    <div><strong>Maskapai</strong><span>{form.airline}</span></div>
                    <div><strong>Bandara</strong><span>{form.airport}</span></div>
                    <div className="ap-preview-price"><strong>Harga</strong><span>{form.price}</span></div>
                  </div>
                </div>
              </div>
              ...
              <div className="ap-preview-titleblock">
                <h4>{form.name || pkg?.name || "Paket Baru"}</h4>
                <div className="ap-preview-date">{form.departure || "10 Desember 2025 - 16 Desember 2025"}</div>
              </div>
