...
import { fetchPackage, savePackage, type PackagePayload, type PackageDetail } from '../../api/packages';
...
export default function AdminPackageEdit() {
    ...
    const loadPackage = async () => {
      ...
      try {
        ...
        if (pkg) {
          ...
          const foods: string[] = [];
          const mosques: string[] = [];
          const transports: string[] = [];

          if (pkg.itinerary && Array.isArray(pkg.itinerary)) {
            pkg.itinerary.forEach((day) => {
              destinations.push(day.destinasi?.join(', ') || '');
              foods.push(day.makan?.join(', ') || '');
              mosques.push(day.masjid?.join(', ') || '');
              transports.push(day.transportasi?.join(', ') || '');
            });
          }
          ...
          setForm({
            ...
            destination: destinations.length > 0 ? destinations : ['', '', ''],
            food: foods.length > 0 ? foods : ['', '', ''],
            mosque: mosques.length > 0 ? mosques : ['', '', ''],
            transport: transports.length > 0 ? transports : ['', '', ''],
          });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await savePackage(payload, id, imageFile || undefined);
    ...
    <AdminLayout title="Edit Paket">
      <div className="bg-white rounded-[18px] px-7 py-6 shadow-[0_8px_24px_rgba(15,23,42,0.12)] border border-[#f0f0f0] max-w-[1200px] w-full mx-auto">
        ...
        <form className="space-y-8">
          {/* Tambah Detail Paket */}
          <div>
            <h2 className="text-lg font-semibold text-gray-800 mb-4">Detail Paket</h2>
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-gray-800">Destinasi Itenary</h2>
          ...
          {/* Tempat Makan Itenary */}
          <div>
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-gray-800">Tempat Makan Itenary</h2>
              <div className="flex gap-2">
                <button
                  type="button"
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-gray-800">Masjid Itenary</h2>
              <div className="flex gap-2">
                <button
                  type="button"
          ...
          {/* Transportasi Itenary */}
          <div>
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-gray-800">Transportasi Itenary</h2>
              <div className="flex gap-2">
                <button
                  type="button"
        ...
        <div className="fixed inset-0 bg-black/50 z-[100] grid place-items-center p-4">
          <div className="bg-white rounded-2xl p-6 max-w-[380px] w-full shadow-[0_20px_50px_rgba(0,0,0,0.3)]">
            <div className="text-center mb-6">
              ...
              <p className="text-gray-600">Paket berhasil diperbarui</p>
            </div>
